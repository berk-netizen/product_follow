...
export function ItemCard({ item, isOverlay }: ItemCardProps) {
    ...
    return (
        ...
        >
            <Card className={cn(
                "shadow-lg hover:shadow-[0_8px_30px_rgba(0,0,0,0.12)] transition-all duration-300 border-white/10 bg-slate-900/60 backdrop-blur-md overflow-hidden group relative",
                isDragging && "ring-2 ring-emerald-500/50 shadow-[0_0_20px_rgba(16,185,129,0.2)]",
                isDelayed && "ring-2 ring-destructive ring-offset-2 ring-offset-slate-900 shadow-[0_0_15px_rgba(220,38,38,0.3)] animate-pulse border-destructive/50"
            )}>
                {isDelayed && (
                    <div className="absolute top-0 right-0 left-0 h-1 bg-destructive pointer-events-none z-10" />
                )}
                {item.image_url && (
                    <div className="relative w-full h-36 overflow-hidden border-b border-white/5">
                        <Image
                            src={item.image_url}
                            alt={item.model_name}
                            fill
                ...
                <CardHeader className="p-4 pb-2 space-y-2">
                    <div className="flex justify-between items-start gap-2">
                        <Badge variant="outline" className="font-mono text-[10px] tracking-tight bg-black/20 text-slate-300 border-white/10">
                            {item.model_code}
                        </Badge>
                        <Badge variant="secondary" className="bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 border-0 text-[10px]">
                            {item.season}
                        </Badge>
                    </div>
                    <CardTitle className="text-base font-semibold leading-tight pt-1 group-hover:text-emerald-400 transition-colors text-white font-playfair">
                        {item.model_name}
                    </CardTitle>
                </CardHeader>
                <CardContent className="p-4 pt-1 space-y-3">
                    <div className="flex items-center justify-between">
                        ...
                        {item.target_sales_price > 0 && (
                            <div className="flex items-center gap-1.5 text-xs font-semibold text-white bg-white/5 rounded-full px-2 py-0.5 border border-white/10">
                    ...
                    <div className="flex items-center justify-between text-xs font-medium bg-black/20 rounded-md p-2 border border-white/5 relative">
                        {isDelayed && (
                        ...
                        <div className="flex items-center gap-2 text-slate-400">
                            ...
                            <span>Target: <strong className="text-slate-200">{targetDateFormatted}</strong></span>
                        </div>
